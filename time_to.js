import { server_data, player_data as multipliers } from "data.js"

/** @param {NS} ns */
export async function main(ns) {
	const [hacking, threads, money] = [80, 600, 56_320_000] // p3 = joesguns??
	// const [hacking, threads, money] = [60, 300, 5_000_000] // p2 = n00dles
	// const [hacking, threads, money] = [40, 135, 1_500_000] // p1 = noodles
	// const [hacking, threads, money] = [1, 59, 700_000] // p0 = noodles
	const best_list = Object.entries(server_data)
		.map(([n, s]) => [n, total_time_to_cash(s, hacking, threads, money)])
		.sort((x, y) => x[1] - y[1]) // low -> high
		.filter(([n, time]) => time < Infinity)
	ns.tprint(best_list)
	// tprint displaying Infinity inside container as null =_=
	// ns.tprint(`isNaN(null) // ${isNaN(null)}`)
}
